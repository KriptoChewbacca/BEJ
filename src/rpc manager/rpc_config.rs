...
impl RpcManagerConfig {
    ...
    pub fn from_toml_file(path: &str) -> Result<Self, ConfigError> {
        let contents = std::fs::read_to_string(path).map_err(|e| {
            ConfigError::IoError(format!("Failed to read config file {}: {}", path, e))
        })?;
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        for endpoint in &self.endpoints {
            ...
            if endpoint.max_concurrency == 0 {
                return Err(ConfigError::ValidationError(format!(
                    "Invalid max_concurrency for {}: must be > 0",
                    endpoint.url
                )));
            }
        }
...
mod tests {
    ...
    fn test_config_validation() {
        // Valid config
        let valid_config =
            RpcManagerConfig::from_urls(&vec!["https://api.mainnet-beta.solana.com".to_string()]);
