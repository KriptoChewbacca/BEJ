...
impl PrometheusMetrics {
    ...
    pub fn add_histogram(&mut self, name: &str, sum: f64, count: u64, buckets: &[(f64, u64)], labels: &[(&str, &str)]) {
        let label_str = Self::format_labels(labels);
        ...
        self.metrics.push(format!(
            "{}_bucket{{{}}} {}",
            name, inf_label_str, count
        ));
        
        // Add sum and count
        self.metrics.push(format!("{}_sum{{{}}} {}", name, label_str, sum));
        self.metrics.push(format!("{}_count{{{}}} {}", name, label_str, count));
    }
...
mod tests {
    ...
    fn test_prometheus_metrics_counter() {
        ...
        metrics.add_counter("rpc_requests_total", 100, &[("endpoint", "test"), ("tier", "tier0")]);
