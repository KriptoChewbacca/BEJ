...
impl NonceManagerApi for IntegratedNonceManager {
    ...
    async fn build_transaction_with_nonce(
        ...
        // Create transaction
        let mut tx = Transaction::new_with_payer(&all_instructions, Some(&payer_pubkey));
...
impl BuyEngineNonceIntegration {
    ...
    pub async fn execute_buy_with_nonce(
        ...
        let lease = self
            .nonce_manager
            .acquire_nonce(Duration::from_secs(5))
            .await?;
        ...
        let _tx = self
            .nonce_manager
            .build_transaction_with_nonce(&lease, instructions)
            .await?;
...
mod tests {
    ...
    async fn test_pool_statistics() {
        ...
        assert_eq!(
            stats.total_nonces,
            stats.available_nonces + stats.leased_nonces
        );
    }
