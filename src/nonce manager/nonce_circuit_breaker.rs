...
impl RLAgent {
    ...
    pub async fn update(&self, state: RLState, action_idx: usize, reward: f64, next_state: RLState) {
...
mod tests {
    ...
    async fn test_global_circuit_breaker_threshold() {
        ...
        global_breaker.locked_nonces_count.store(71, Ordering::Relaxed);
        assert!(global_breaker.should_open(100));
        
        // Should open with high latency
        global_breaker.locked_nonces_count.store(0, Ordering::Relaxed);
