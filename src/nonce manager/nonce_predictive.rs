...
impl UniversePredictiveModel {
    ...
    pub fn record_refresh_full(&mut self, slot: u64, latency_ms: f64, tps: u32, slots_consumed: u64) {
    ...
    pub fn record_refresh_with_volume(&mut self, slot: u64, latency_ms: f64, tps: u32, volume_sol: f64, slots_consumed: u64) {
        // Validate inputs
        ...
        self.history.push_back((slot, clipped_latency, tps, clipped_volume));
    ...
    fn normalize_slot(&self, slot: u64) -> f64 {
        ...
        let normalized = (slot - self.slot_range.0) as f64 / (self.slot_range.1 - self.slot_range.0) as f64;
    ...
    pub fn predict_failure_probability(&mut self, network_tps: u32) -> Option<f64> {
        ...
        let ema_prob = (0.4 * latency_risk + 0.3 * congestion_risk + 0.3 * slot_risk).clamp(0.0, 1.0);
        ...
        let probability = (0.3 * ema_prob + 0.3 * regression_prob + 0.4 * lstm_prob).clamp(0.0, 1.0);
    ...
    fn update_q_value(
        ...
        let max_next_q = actions.iter().map(|(_, _, q)| *q).fold(f64::NEG_INFINITY, f64::max);
        
        // Q-learning update: Q(s,a) = (1-α)Q(s,a) + α(r + γ*max Q(s',a'))
        let current_q = actions[best_match_idx].2;
        let new_q = (1.0 - self.rl_alpha) * current_q + self.rl_alpha * (reward + self.rl_gamma * max_next_q);
    ...
    pub fn get_stats(&self) -> ModelStats {
        ...
        for pred in self.predictions.iter().rev().take(100) {
            if let (Some(actual_success), Some(actual_latency)) = (pred.actual_success, pred.actual_latency_ms) {
                total += 1;
        ...
        ModelStats {
            ...
            has_sufficient_data: self.latency_history.len() >= self.min_sample_size && variance > 10.0,
    ...
    pub fn set_lstm_weights(&mut self, weights: HashMap<String, Vec<f64>>) {
        ...
        debug!(
            num_layers = self.lstm_weights.len(),
            "Loaded LSTM weights"
        );
    }
