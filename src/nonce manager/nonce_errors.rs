...
impl std::fmt::Display for UniverseErrorType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            UniverseErrorType::ValidatorBehind { slots } => write!(f, "Validator behind by {} slots", slots),
            ...
            UniverseErrorType::PredictiveFailure { probability } => write!(f, "Predictive failure (p={:.2})", probability),
            UniverseErrorType::SecurityViolation { reason } => write!(f, "Security violation: {}", reason),
            UniverseErrorType::QuotaExceeded => write!(f, "Quota exceeded"),
            UniverseErrorType::ClusterCongestion { tps } => write!(f, "Cluster congestion (tps={})", tps),
            UniverseErrorType::ClusteredAnomaly { cluster_id, confidence } => write!(f, "Clustered anomaly (cluster={}, conf={:.2})", cluster_id, confidence),
        }
    }
}
