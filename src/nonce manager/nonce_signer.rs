...
pub trait SignerService: Send + Sync {
    ...
    async fn batch_verify_signatures(&self, signatures: &[Signature], messages: &[Vec<u8>]) -> NonceResult<bool> {
...
impl SignerService for MockSigner {
    ...
    async fn sign_transaction(&self, _transaction: &mut Transaction) -> NonceResult<()> {
        if self.should_fail {
            Err(NonceError::Signing("Mock signer configured to fail".to_string()))
        } else {
...
impl SignerService for HardwareWalletSigner {
    ...
    async fn sign_advance(&self, nonce_account: &Pubkey, _transaction: &mut Transaction) -> NonceResult<()> {
        ...
        Err(NonceError::Signing(
            format!("Ledger nonce advance not yet implemented for account {}", nonce_account)
        ))
    }
...
mod tests {
    ...
    async fn test_local_signer() {
        ...
        let mut tx = Transaction::new_with_payer(
            &[system_instruction::transfer(&pubkey, &Pubkey::new_unique(), 1000)],
            Some(&pubkey),
        );
