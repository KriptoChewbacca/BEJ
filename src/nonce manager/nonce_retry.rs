use super::nonce_errors::{ErrorClassification, NonceError, NonceResult, UniverseErrorType};
use rand::Rng;
    ...
    Fut: Future<Output = NonceResult<T>>,
{
    ...
    // All attempts failed
    Err(last_error
        .unwrap_or_else(|| NonceError::Internal("Retry exhausted without error".to_string())))
}
...
impl ErrorClassifier {
    ...
    fn classify_by_pattern(&self, error_str: &str) -> ErrorClassification {
        ...
        if error_lower.contains("behind")
            || error_lower.contains("slot") && error_lower.contains("lag")
        {
            return ErrorClassification {
        ...
        if error_lower.contains("unauthorized")
            || error_lower.contains("signature") && error_lower.contains("invalid")
        {
            return ErrorClassification {
        ...
        ErrorClassification {
            error_type: UniverseErrorType::Base(Box::new(NonceError::Internal(
                error_str.to_string(),
            ))),
            confidence: 0.5,
            is_transient: false,
    ...
    async fn calculate_cluster_confidence(&self, cluster_id: u8) -> f64 {
        let history = self.history.read().await;
        let cluster_size = history
            .iter()
            .filter(|e| e.cluster_id == Some(cluster_id))
            .count();
    ...
    for attempt in 0..config.max_attempts {
        ...
        if let Some(cb) = breaker {
            if !cb.can_execute().await {
                ...
                return Err(NonceError::Internal("Circuit breaker open".to_string()));
            }
        }
...
mod tests {
    ...
    async fn test_retry_succeeds_on_first_attempt() {
        ...
        let result =
            retry_with_backoff("test_op", &config, || async { Ok::<i32, NonceError>(42) }).await;
    ...
    async fn test_retry_exhausts_all_attempts() {
        ...
        let result: Result<(), NonceError> = retry_with_backoff("test_op", &config, || {
            let _ = attempt_count_clone.fetch_add(1, Ordering::SeqCst);
            async { Err(NonceError::Timeout(1000)) }
        })
        .await;
    ...
    async fn test_retry_with_metrics() {
        let config = RetryConfig::default();
        let (result, metrics) =
            retry_with_metrics("test_op", &config, || async { Ok::<i32, NonceError>(42) }).await;
