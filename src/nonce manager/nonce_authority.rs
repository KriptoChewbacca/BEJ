...
impl AuthorityRotationManager {
    ...
    pub async fn execute_rotation(
        ...
        let mut tx = Transaction::new_with_payer(&[ix], Some(&current_authority_keypair.pubkey()));
    ...
    #[instrument(skip(self))]
    pub async fn rollback_rotation(&self, proposal_id: &str, reason: String) -> NonceResult<()> {
        let mut proposals = self.proposals.write().await;
        let proposal = proposals
            .iter_mut()
            .find(|p| p.id == proposal_id)
...
mod tests {
    ...
    async fn test_rotation_approval() {
        ...
        manager
            .approve_rotation(&proposal_id, approver1, "sig1".to_string())
            .await
            .unwrap();
        ...
        manager
            .approve_rotation(&proposal_id, approver2, "sig2".to_string())
            .await
            .unwrap();
    ...
    async fn test_audit_log() {
        ...
        assert!(matches!(
            audit_log[0].event_type,
            RotationEventType::Proposed
        ));
    }
}
