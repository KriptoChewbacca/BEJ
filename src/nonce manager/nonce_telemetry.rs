...
impl LatencyHistogram {
    ...
    pub async fn export_prometheus(&self, name: &str) -> String {
        ...
        format!(
            ...
            name, percentiles.count,
            name, percentiles.count,
            name, percentiles.p50 * percentiles.count as f64,
            name, percentiles.count,
        )
    }
}
...
impl NonceTelemetry {
    ...
    pub async fn record_refresh(&self, success: bool, latency: Duration) {
        self.counters.refresh_attempts_total.fetch_add(1, Ordering::Relaxed);
        if !success {
            self.counters.refresh_failures_total.fetch_add(1, Ordering::Relaxed);
        }
    ...
    async fn check_refresh_failure_rate(&self) {
        ...
        if failure_rate > 0.05 {
            self.alerting.trigger_alert(Alert {
                ...
                message: format!("Refresh failure rate {:.2}% exceeds threshold", failure_rate * 100.0),
    ...
    pub async fn check_latency_alerts(&self) {
        ...
        if percentiles.p99 > 0.100 {
            self.alerting.trigger_alert(Alert {
                severity: AlertSeverity::Warning,
                message: format!("Acquire P99 latency {:.3}s exceeds threshold", percentiles.p99),
    ...
    pub async fn export_prometheus(&self) -> String {
        ...
        output.push_str("\n");
        output.push_str(&self.acquire_latency.export_prometheus("nonce_acquire_latency_seconds").await);
        output.push_str("\n");
        output.push_str(&self.refresh_latency.export_prometheus("nonce_refresh_latency_seconds").await);
