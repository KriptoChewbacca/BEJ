...
mod phase1_nonce_enforcement_tests {
    ...
    async fn test_default_critical_sniper_priority_when_enforced() {
        ...
        let should_upgrade =
            enforce_nonce && matches!(config.operation_priority, OperationPriority::Utility);
        assert!(
            should_upgrade,
            "Should upgrade Utility to CriticalSniper when enforce_nonce=true"
        );
    }
    ...
    async fn test_no_priority_upgrade_when_not_enforced() {
        ...
        assert!(
            !should_upgrade,
            "Should not upgrade when enforce_nonce=false"
        );
    }
    ...
    async fn test_no_upgrade_for_critical_sniper() {
        ...
        assert!(
            !should_upgrade,
            "Should not upgrade when already CriticalSniper"
        );
    }
    ...
    async fn test_enforce_nonce_parameter_exists() {
        ...
        assert!(
            true,
            "Config parameters for nonce enforcement are accessible"
        );
    }
    ...
    async fn test_config_validation_with_ttl() {
        ...
        match config.validate() {
            ...
            Err(e) => {
                ...
                println!(
                    "Config validation returned error (expected for partial config): {}",
                    e
                );
            }
        }
