...
mod nonce_lease_tests {
    ...
    async fn test_lease_expiry_detection() {
        ...
        assert!(!lease.is_expired(), "Lease should not be expired immediately");
        assert!(lease.time_remaining().is_some(), "Should have time remaining");
        
        // Wait for expiry
        tokio::time::sleep(Duration::from_millis(150)).await;
        
        assert!(lease.is_expired(), "Lease should be expired after TTL");
        assert!(lease.time_remaining().is_none(), "Should have no time remaining");
    }
    ...
    async fn test_lease_auto_release_on_drop() {
        ...
        assert_eq!(released.load(Ordering::SeqCst), 1, "Should be auto-released on drop");
    }
    ...
    async fn test_watchdog_reclaims_expired_lease() {
        ...
        assert!(expired_count.load(Ordering::SeqCst) > 0, "Watchdog should detect expiry");
        assert_eq!(watchdog.active_lease_count().await, 0, "Lease should be removed");
