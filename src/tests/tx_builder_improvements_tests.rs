...
mod tx_builder_improvements_tests {
    use crate::tx_builder::{OperationPriority, QuorumConfig, TransactionConfig};
    ...
    fn test_operation_priority_fallback() {
        ...
        assert_eq!(
            OperationPriority::CriticalSniper.allow_blockhash_fallback(),
            false
        );
    ...
    fn test_simulation_error_classification() {
        ...
        let advisory_errors = vec!["Some other warning", "Advisory message"];
        ...
        for error in advisory_errors {
            assert!(
                !error.contains("InstructionError")
                    ...
                    && !error.contains("ComputeBudgetExceeded")
                    && !error.contains("InsufficientFunds"),
                "Should be classified as advisory: {}",
                error
            );
        }
    }
    ...
    fn test_adaptive_priority_fee_calculation() {
        ...
        assert_eq!(
            adaptive_priority_fee, 15_000,
            "Should apply multiplier correctly"
        );
        ...
        assert_eq!(
            adaptive_priority_fee, 10_000,
            "Should return base with 1.0 multiplier"
        );
    }
}
