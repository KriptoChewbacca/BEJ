...
mod simulation_nonce_tests {
    ...
    fn build_simulation_instructions(program_id: Pubkey) -> Vec<Instruction> {
        vec![
            ...
            // Program instruction
            Instruction::new_with_bytes(program_id, &[1, 2, 3, 4], vec![]),
        ]
    }
    ...
    fn test_simulation_excludes_advance_nonce() {
        ...
        assert!(
            !has_advance_nonce,
            "Simulation should not include advance_nonce"
        );
    ...
    fn test_execution_includes_advance_nonce() {
        ...
        let exec_instructions =
            build_execution_instructions(&nonce_account, &nonce_authority, program_id);
    ...
    fn test_simulation_execution_program_instructions_match() {
        ...
        let sim_program_ixs: Vec<_> = sim_instructions
            .iter()
            .filter(|ix| {
        ...
        let exec_program_ixs: Vec<_> = exec_instructions
            .iter()
            .filter(|ix| {
                ix.program_id != solana_sdk::compute_budget::id()
                    && ix.program_id != solana_sdk::system_program::id()
    ...
    async fn test_simulation_with_nonce_context_no_advance() {
        ...
        assert!(
            !has_advance_nonce,
            "Simulation should exclude advance_nonce"
        );
    ...
    async fn test_execution_with_nonce_context_advances_nonce() {
        ...
        let program_id = Pubkey::new_unique();
        let exec_instructions =
            build_execution_instructions(&nonce_pubkey, &nonce_authority, program_id);

        // Verify advance_nonce is first instruction
        assert!(!exec_instructions.is_empty());
        assert_eq!(
            exec_instructions[0].program_id,
    ...
    fn test_simulation_execution_instruction_count_difference() {
        ...
        assert_eq!(
            exec_instructions.len(),
            sim_instructions.len() + 1,
