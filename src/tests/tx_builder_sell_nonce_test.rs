...
mod tx_builder_sell_nonce_test {
    ...
    fn test_sell_transaction_respects_operation_priority() {
        ...
        assert!(
            critical.requires_nonce(),
            "CriticalSniper should require nonce"
        );
        assert!(
            !critical.allow_blockhash_fallback(),
            "CriticalSniper should not allow fallback"
        );

        // Utility should prefer blockhash
        let utility = OperationPriority::Utility;
        assert!(
            !utility.requires_nonce(),
            "Utility should not require nonce"
        );
        assert!(
            utility.allow_blockhash_fallback(),
            "Utility should allow fallback"
        );
        ...
        assert!(
            bulk.allow_blockhash_fallback(),
            "Bulk should allow fallback"
        );
    }
